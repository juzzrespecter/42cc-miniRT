...
/*   Updated: 2020/08/24 18:05:26 by danrodri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void transform_vectors(float *height, float *width, float *world_or)
{
	...
	while (count < 4)
		{
			height_obj[count] = 1 * (count == 1);
			width_obj[count] = 1 * (count == 0);
			object_orientation[count] = 1 * (count == 2);
			count++;
		}
	obj2world_matrix(object_orientation, world_orientation, transformation_matrix);
...
t_3dvec *collision_square(t_sq *sq, t_3dvec *ray)
{
	...
	if (!(point_in_plane(sq->vector, ray, dot(sq->vector, sq->coord), point)))
		return (NULL);

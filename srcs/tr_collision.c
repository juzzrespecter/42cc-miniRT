...
static void edge_vp_cross_prod(float *v1, float *v2, float *point, float *cross)
{
  float edge[3];
  float vp[3];
  int count;
...
t_3dvec *tr_collision(t_tr *tr, t_3dvec *ray)
{
  float normal[3];
  float point[3];
  float d;
  ...
  if (!(triangle_point_check(tr, point, normal)))
    return (NULL);
  found_point(point_coord, normal, tr->color, ray);
}
